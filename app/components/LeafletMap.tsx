...
export default function LeafletMap({
  posts, center, zoom, isSmartSearch, router, formatDateTime, getBehaviorLabel
}: LeafletMapProps) {
  if (typeof window === "undefined") return null;
  ...
  return (
    <MapContainer center={center} zoom={zoom} className="w-full h-full z-0">
      ...
      {posts.map((post) => {
        ...
        const icon = getLeafletIcon(post.animal_type, isSmartSearch && post.matchScore > 70);

        // --- เตรียมข้อมูลรูปภาพสำหรับ Slider ---
        const imagesList = post.images && post.images.length > 0
          ? post.images.map((img: any) => typeof img === "string" ? img : img.image_url)
          : ["https://via.placeholder.com/300x200.png?text=No+Image"];

        return (
          <Marker key={post.report_id} position={[lat, lng]} icon={icon || undefined}>
              ...
              <div className="flex flex-col w-full bg-white overflow-hidden">
                ...
                <div className="relative w-full h-35 overflow-hidden group">
                  ...
                  {imagesList.length > 1 && (
                    <>
                      <button 
                        onClick={() => document.getElementById(`slider-${post.report_id}`)?.scrollBy({ left: -200, behavior: 'smooth' })}
                  ...
                  <div className="absolute top-2 left-2 bg-black/60 backdrop-blur-md px-2 py-1 rounded-full z-10 flex items-center gap-1">
                    <div className={`w-1.5 h-1.5 rounded-full ${
                      post.status === "STILL_THERE" ? "bg-red-500" : post.status === "MOVED" ? "bg-amber-500" : "bg-green-500"
                    }`} />
                ...
                <div className="p-3">
                  <h3 className="text-sm font-bold text-gray-900 leading-tight mb-1">พบที่ {post.location}</h3>
                  <p className="text-[10px] text-gray-400 mb-2">
                    {formatDateTime(post.created_at)} • โดย {post.user?.name || "ไม่ระบุ"}
                  </p>
                  
                  <div className="bg-gray-50 border border-gray-100 rounded-lg p-2 mb-3 text-[11px] text-gray-700 space-y-1">
                    <p><span className="text-gray-400">ลักษณะ:</span> {post.description || "ไม่มีคำอธิบาย"}</p>
                    <p><span className="text-gray-400">พฤติกรรม:</span> {getBehaviorLabel(post.behavior)}</p>
                  </div>

                  {/* ปุ่ม Action */}
                  <div className="flex gap-2 mb-2">
                    <button 
                      onClick={() => (window as any).handleHelpAction(post.report_id, 'FEED')}
                    ...
                    >ให้อาหาร</button>
                    <button 
                      onClick={() => (window as any).handleHelpAction(post.report_id, 'ADOPT')}
                      className="flex-1 h-9 bg-[#D4A373] text-white rounded-xl font-bold text-xs hover:bg-[#c49261]"
                    >รับเลี้ยง</button>
                  </div>

                  <button 
                    onClick={() => router.push(`/animal-report/${post.report_id}`)}
