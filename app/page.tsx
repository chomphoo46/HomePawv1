  ...
  const handleSmartSearch = () => {
    ...
    const scoredPosts = allAnimalPosts.map((post) => {
      ...
      if (
        ...
        post.description
          ?.toLowerCase()
          .includes(searchCriteria.keyword.toLowerCase())
      )
        score += 30;
  ...
  return (
    <div className={`min-h-screen bg-white text-gray-800 ${mali.className}`}>
          ...
          <div className="flex flex-col justify-center items-center p-8 order-2 md:order-1">
            <div className="max-w-lg">
              ...
              <p className="text-gray-600 mb-8">
                หากคุณพบสัตว์ที่ถูกทอดทิ้ง หรือเจ็บป่วย
                ทุกการแจ้งคือโอกาสใหม่ของหนึ่งชีวิต
              </p>
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 min-h-125">
          ...
          <div className="flex flex-col justify-center items-center p-8">
            <div className="max-w-lg">
              ...
              <p className="text-gray-600 mb-8">
                เราขอช่วยเป็นสะพานให้สัตว์เลี้ยงที่คุณรักได้พบกับเจ้าของคนใหม่ที่อบอุ่น
              </p>
              <button
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 gap-4 text-center py-14 px-4">
        ...
        <div>
          <div className="bg-orange-100 text-[#D4A373] w-14 h-14 rounded-full flex items-center justify-center mb-4 mx-auto">
            <PawPrint className="w-7 h-7" />
          </div>
          <p className="text-3xl font-bold text-[#D4A373] mb-1">
            {stats.foundAnimals}
          </p>
          <p className="text-gray-600">สัตว์ไร้บ้านที่พบ</p>
        </div>
        <div>
          <div className="bg-green-100 text-green-600 w-14 h-14 rounded-full flex items-center justify-center mb-4 mx-auto">
        ...
        <div className="p-6 text-black bg-gray-50 rounded-3xl">
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex-2 relative w-full">
              <input
                type="text"
                placeholder="ระบุลักษณะ..."
                className="w-full rounded-xl px-4 py-3 pl-10 border outline-none"
            ...
            <button
              ...
              className="bg-[#D4A373] text-white font-bold px-6 py-3 rounded-xl shadow-md flex items-center justify-center gap-2"
            >
              <BiTargetLock /> ค้นหา
            </button>
            {isSmartSearchActive && (
              <button
                onClick={handleReset}
                className="bg-white text-black px-6 py-3 rounded-xl border shadow-md"
              >
                ล้างค่า
              </button>
            )}
      ...
      <section className="px-4 py-8">
        ...
        <div className="text-center mt-8">
          <button
            ...
            className="px-6 py-2 border-2 border-[#D4A373] text-[#D4A373] rounded-full font-bold hover:bg-[#D4A373] hover:text-white transition-all"
          >
            ดูประกาศทั้งหมด →
          </button>
        </div>
