    ...
    const fetchPost = async () => {
      try {
        ...
        setFormData({
          ...
          dateTime: new Date(data.created_at).toISOString().slice(0, 16),
          images: [],
        });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
       deletedImageIds,
        // สำหรับ images เราจะต้อง upload ก่อนแล้วส่ง URL กลับไปให้ API
        // ตัวอย่างนี้สมมติว่า formData.images เป็น File[] และคุณมี backend ที่รองรับ multipart/form-data
      };
      ...
      router.push(`/rehoming-report/${post_id}`);
    } catch (err) {
      console.error(err);
    }
  };
